...
For the first feature set I use three spatially binned color channels. The method [`bin_spatial(self, img, size=(32, 32))`](CoreImageProcessing.py) first resizes the `img` (bounding box) to the `size` and concatenates the raw pixel values to an 1D feature vector.
...
In order to find the best hyper-parameters for the color histogram I applied the same method as described in chapter [3.2 Spatially Binned Features](#33-spatially-binned-features) above. The diagram below shows all accuracies on the test dataset.
...
The last feature set is based on the histogram of oriented gradients (HOG) method. The method [`hog_features(self, img, channel, orient, pix_per_cell, cell_per_block, vis=False, feature_vec=True)`](CoreImageProcessing.py) calculates the HOG features on the given `img` for the specified color `channel` and returns a 1D feature vector. For further details about the HOG calculation have a look to the paper [Histograms of Oriented Gradients for Human Detection, Navneet Dalal and Bill Triggs](http://lear.inrialpes.fr/people/triggs/pubs/Dalal-cvpr05.pdf).
...
After tuning of the individual methods like spatial binning, color histograms and HOG features I combined the best three feature sets and fine tuned its hyper-parameters in order to find an optimum between the accuracy and the computing time. The parameter setup in the YCrCb color space with 16 orientations in the HOG method achieved the best accuracy of 99,5 %. But due to the high computing time I chose the second best results with 99,25 % which almost needs 1/4 less computing time. The whole feature extraction pipeline has been implemented in the method [`extract_features(self, img_rgb)`](VehicleClassifier.py).
...
First I tried the simple sliding window approach which turned out to be extremely slow, especially when you apply multi-scaled windows. Therefore, I decided to implement the "HOG sub-sampling method" (see [`find_vehicles(self, img_rgb, y_start, y_stop, scale, cells_per_step)`](VehicleDetection.py)). The basic idea of this method is that you calculate the computational intensive HOG function once on a cropped and  scaled down image. The ROI is defined by `y_start` to `y_stop` and the scaling factor by `scale`. Afterwards, the scaled image is sub-sampled to calculate the other feature sets like spatial binning and color histogram.
...
The source code can be found in [`find_vehicles(self, img_rgb, y_start, y_stop, scale, cells_per_step)`](VehicleDetection.py):
...
The heart of the vehicle detection pipeline is located in the [`detect_vehicles(self, img_rgb)`](VehicleDetection.py) method. This method consist of the following processing steps.
...
This project uses kind of old fashioned methods. However it's was extremely valuable go through all these processing steps like HOG, spatial binning, color histogram, heatmaps, etc. to get a better feeling how difficult and time consuming the manual parameter tuning process could be. Futhermore, this experience helps to judge the pros and cons of newer or alternatives concepts.

My pipeline works well on the used video clip but is far away from a function that could be used in a real product. The pipeline e.g has problems when two vehicles in the image are located too close to each other. In that case the bounding boxes of these vehilces are merged together. Possible solutions would be to classify the color of each vehicle or to estimate its position in world coordinates and compare its relative speeds to better distinguish them.
